...
% title
\title{
    Soil microbiome meta-analysis combined with remote sensing and spatial data,
    identifies soil health and desertification indicators, facilitating semi-automated assessment in Crete, Greece
}
% authors
\author{
...
potential desertification (PD), light desertification (LD), 
severe desertification (SD), and  very severe desertification (VSD), 
with vegetation coverage 75\%, 70\%, 43\%, and 6\%, respectively \parencite{Fan2020Soil}.


\bigbreak

In European Mediterranean drylands, microbial biomass (but not diversity) was suggested as 
the most appropriate soil biological index to indicate the negative effect of desertification 
on soil biology \parencite{Valentina2022Soil}.



\begin{itemize}
    \item Soil microbial biomass is positively correlated to soil organic carbon (SOC) and total nitrogen (TN)
    \item Bacterial diversity is negatively correlated to SOC
    \item pH was the most influencing factor positively affecting bacterial alpha-diversity
    \item A core microbiota was identified in semi-arid soils of Southern Europe % arid = άνυδρος
    \item Baseline data for the monitoring of EU soils facing desertification
\end{itemize}


\bigbreak

\parencite{Neilson2017Significant}
\begin{itemize}
    \item soil microbiomes exhibit a gradient in composition, are distinct from a broad cross-section of nondesert soils, 
    and yet are similar to three deserts from different continents
    \item Community richness and diversity were significantly positively correlated with soil relative humidity (SoilRH)
    \item The strongest and most significant correlations between SoilRH and phylum relative abundance were observed for Acidobacteria, 
    Proteobacteria, Planctomycetes, Verrucomicrobia, and Euryarchaeota
    \item Network analysis revealed a deterioration in the density of significant associations between taxa along the arid to hyperarid gradient, 
    a pattern that may compromise the resilience of hyperarid communities because they lack properties associated with communities 
...
\parencite{Lima2023Grazing}
Biomass-based metrics Proposed as primary biological index


Bulk soil density and sodium adsorption ratio (SAR) were the indicators that presented a higher negative correlation with soil health (SH), 
mainly in degraded soil by overgrazing (DE), while SOC, microbial biomass carbon (MBC), and $\beta$-glucosidase activity correlated with SH 
in \textit{native vegetation} and \textit{restored soil by grazing exclusion} soils. 
Biological soil health indicators increased in dry season, which may be due to the deciduous behavior of Caatinga vegetation, 
which could intensify microbial activity. 



grazing exclusion, cover crops, and terracing, have been implemented in the Brazilian semiarid. 
These practices have shown promising results in terms of enhancing soil fertility and restoring microbial properties. 


\bigbreak

Beyond microbiome, there is a great number of studies using remote sensing and spatial data to assess desertification and land degradation,
indicatively \parencite{liu2026remote, wu2026decoding}.
...
for assessing land degradation and desertification in semi-arid regions using LANDSAT 8. 

Landsat is defined as the oldest continuous Earth-observing satellite imaging program, launched by NASA and the USGS, which provides 30m 
resolution optical and thermal infrared satellite imagery globally.

Iranian Model for Desertification Potential Assessment (IMDPA) in 2006.
...
Soil microbiome composition is a sensitive and promising component of desertification assessment, 
but on current evidence it is best used as part of a multi-indicator framework, not as a single, globally reliable desertification index.

% climate - ariditiy index 
% soil - soil salinity (gourndwater changes, water table fluctuation, groundwater quality)
% vegetation - land cover changes assessment using the NDVI index
% agriculture
% groundwater changes
% socioeconomic factors - population density, 
% development of urbal-industrial areas - land use changes assessment usind the NDVI index

Desertification is a complex process shaped by interacting climatic (e.g., aridity index), 
soil (e.g., salinity, groundwater dynamics), vegetation (e.g., land cover changes assessed via NDVI), 
agricultural, hydrological, and socioeconomic factors (e.g., population density, urban–industrial expansion).
Soil microbiome composition is a sensitive and promising indicator of desertification. 
However, current evidence suggests it is most effective when integrated within a multi-indicator framework rather than used as a standalone, 
globally reliable index. 
Incorporating soil microbiome data into such frameworks can enhance sensitivity, provide early-warning signals, 
and improve the overall accuracy and frequency of desertification assessment.

Here, we apply a data integration approach to combine soil microbiome data with remote sensing and spatial data to 
identify soil health and desertification indicators in Crete, Greece.
This approach can facilitate semi-automated assessment of soil health and desertification risk, 
providing valuable insights for land management and conservation efforts in the region.
